...
t_j = zeros(K,1);
for i=1:m
	t_j = t_j + (-1*mapped_y(:,y(i)) .* log(a3(i,:).') - (1 - mapped_y(:,y(i))) .* log(1 - a3(i,:).'));
end

J = sum(t_j)/m;
y_k = eye(K);
regularizedResult = sum(sum(Theta1(:,2:end) .^2)) + sum(sum(Theta2(:,2:end).^2)); 
J = J + regularizedResult * lambda / (2 * m);
